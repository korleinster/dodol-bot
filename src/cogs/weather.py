...
class Weather(commands.Cog):
    ...
    async def daily_weather(self):
        print(f"[뚠뚠봇{self.bn:03d}] [Weather] 자동 날씨 발송 시작")
        embed = await _fetch_weather_embed()
        if embed is None:
            print(f"[뚠뚠봇{self.bn:03d}] [Weather] embed 생성 실패 — API 오류")
            return
        embed.set_footer(text="매일 07:00 자동 발송")

        ch_ids = await self._get_all_channels()
        print(f"[뚠뚠봇{self.bn:03d}] [Weather] 대상 채널: {ch_ids}")
        for ch_id in ch_ids:
            ...
            if ch is None:
                try:
                    ...
                    print(f"[뚠뚠봇{self.bn:03d}] [Weather] 채널 fetch 실패 ch={ch_id}: {e}")
                    continue
            if isinstance(ch, discord.TextChannel):
                try:
                    await ch.send(embed=embed)
                    print(f"[뚠뚠봇{self.bn:03d}] [Weather] 발송 완료 ch={ch_id}")
                except Exception as e:
                    print(f"[뚠뚠봇{self.bn:03d}] [Weather] 자동 발송 실패 ch={ch_id}: {e}")
