...
class Setup(commands.Cog):
    ...
    async def _cmd_list_bots(self, message: discord.Message):
        ...
        if not rows:
            await message.channel.send("아직 소환된 봇이 없습니다. `소환 뚠뚠봇001` 으로 이 채널에 봇을 배치하세요.")
            return
        ...
        for r in sorted(rows, key=lambda x: x["bot_number"]):
            ...
            lines.append(
                f"**뚠뚠봇{n:03d}**  |  명령: {tch.mention if tch else '미설정'}  |  음성: {vch.name if vch else '미설정'}"
            )

        embed = discord.Embed(title="📋 뚠뚠봇 배치 현황", description="\n".join(lines), color=0x5865F2)
        await message.channel.send(embed=embed)

    # ── .소환 nnn ─────────────────────────────────────────

    async def _cmd_summon(self, message: discord.Message, target: str):
        """뚠뚠봇nnn 을 현재 음성/텍스트 채널로 배치"""
        try:
            target_num = int(target.removeprefix("뚠뚠봇").lstrip("0") or "0")
        except ValueError:
            return
    ...
    async def _cmd_status(self, message: discord.Message):
        ...
        if not cfg:
            await message.channel.send(f"뚠뚠봇{self.bn:03d} 은 아직 배치되지 않았습니다. `소환 뚠뚠봇{self.bn:03d}` 을 입력하세요.")
            return
